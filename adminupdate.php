...
if (isset($_SESSION['admin'])){
	...
	if ( !empty($_POST)) 
	{
		...
		if ($valid) {
			...
			$sql = "UPDATE festival SET fest_name = ?, fest_detail = ?, fest_date = ?, fest_time = ?, fw_number = ?, access = ?, location = ?, price = ?, fest_img1 = ?, fest_img2 = ?, map = ?, fest_islandfk = ? WHERE fest_id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($festname, $festdetail, $festdate, $festtime, $festfwnum, $festaccess, $festlocation, $festprice, $festimage1, $festimage2, $festmap, $festisland, $festid));
			Database::disconnect();
			header("Location: adminshow.php");
		}
	} else {
		...
		$festname = $data['fest_name'];
		$festdetail = $data['fest_detail'];
        $festdate = $data['fest_date'];
        $festtime = $data['fest_time'];
        $festfwnum = $data['fw_number'];
        $festaccess = $data['access'];
        $festlocation = $data['location'];
        $festprice = $data['price'];
        $festimage1 = $data['fest_img1'];
        $festimage2 = $data['fest_img2'];
        $festmap = $data['map'];
        $festisland = $data['fest_islandfk'];
		Database::disconnect();
	}
?>
<!DOCTYPE html>
...
<body>
    <div class="container">
    			<div class="span10 offset1">
    				<div class="row">
		    			<h3>Update a Festival</h3>
		    		</div>
    		
	    			<form class="form-horizontal" action="adminupdate.php?festid=<?php echo $festid?>" method="post">
					  ...
					  <div class="control-group <?php echo !empty($festdetailError)?'error':'';?>">
                        ...
                        <div class="controls">
                            <textarea name="festdetail"><?php echo !empty($festdetail)?$festdetail:'';?></textarea>
